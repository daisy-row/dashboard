...
const App: React.FC = () => {
  ...
  const processedData = useMemo(() => {
    ...
    return rawProjects.map(project => {
      const filteredRepos = project.repos.map(repo => {
        if (repoSearchTerm && !repo.name.toLowerCase().includes(repoSearchTerm.toLowerCase())) {
          return null;
        }
        ...
        const filteredEvents = repo.events.filter(e => {
          ...
          if (userSearchTerm && !e.u.toLowerCase().includes(userSearchTerm.toLowerCase())) return false;
          return true;
        });

        if (filteredEvents.length === 0 && (start || end || userSearchTerm || repoSearchTerm)) {
          return null;
        }
  ...
  }, [rawProjects, startDate, endDate, searchTerm, userSearchTerm, repoSearchTerm]);
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
          <div className="flex items-center space-x-2">
            <div className="bg-blue-600 p-2 rounded-lg cursor-pointer" onClick={() => setSelectedProjectName(null)}>
              <Activity className="text-white w-6 h-6" />
            </div>
            <h1 className="text-xl font-bold text-gray-900 tracking-tight cursor-pointer" onClick={() => setSelectedProjectName(null)}>Project Activity Dashboard</h1>
          </div>
          <div className="flex items-center space-x-4">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="pl-10 pr-4 py-2 bg-gray-100 border-transparent focus:bg-white focus:ring-2 focus:ring-blue-500 rounded-lg text-sm transition-all outline-none w-40"
              />
            </div>
            <div className="relative">
            ...
            <div className="bg-white rounded-xl shadow-sm p-8 border border-gray-100">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
                ...
                <div className="bg-gray-50 rounded-lg p-6">
                  ...
                  <div className="h-80 min-w-0 min-h-0 flex items-center gap-4">
                    ...
                    <ul className="shrink-0 max-h-full overflow-y-auto space-y-1 text-[10px] pr-1">
                      {selectedProject.activityTypes.map((t, index) => (
                        <li key={t.name} className="flex items-center">
                          <span
                            className="w-3 h-3 rounded-sm mr-2 shrink-0"
                            style={{ backgroundColor: COLORS[index % COLORS.length] }}
                          />
                          <span style={{ color: COLORS[index % COLORS.length] }}>{t.name}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {selectedProject.repos.map((repo, i) => (
                  <div key={i} className="border border-gray-100 rounded-lg p-5 hover:shadow-md transition-all bg-white">
                    <h4 className="font-bold text-gray-900 mb-3 flex items-center border-b border-gray-50 pb-2">
